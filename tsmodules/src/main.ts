...
let EchoRelayGetAccount = function (ctx: nkruntime.Context, logger:
	nkruntime.Logger, nk:
...
function (ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, payload: string) {
	...
	let account = EchoRelayGetAccount(ctx, logger, nk, userId);

	return JSON.stringify(account);
}

let echoRelaySetAccountRpc: nkruntime.RpcFunction =
	_storageWriteRequest('profile', 'server', account.profile.client),
	function (ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, payload: string) {
		...
		let _storageWriteRequest = ({c, k, v}) =>  ({
			collection: c,
			key: k,
			userId, value: v,
			// server 
		...
		let newObjects: nkruntime.StorageWriteRequest[] = [
			_storageWriteRequest('profile', 'client', account.profile.client),
			_storageWriteRequest('profile', 'server', account.profile.client),
			_storageWriteRequest('relayConfig', 'authSecrets', account.profile.client),
		];
